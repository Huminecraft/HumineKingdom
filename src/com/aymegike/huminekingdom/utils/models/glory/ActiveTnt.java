...
public class ActiveTnt extends GloryEvent {
	...
	public void onPlayerBeGlorious(Player player, Event event) {
		...
		if (player.getInventory().getItemInMainHand().getType() == Material.FLINT_AND_STEEL && e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock() != null && e.getClickedBlock().getType() == Material.TNT) {
			...
			if (!isInAPersonalZone) {
				kingdom.setGlory(kingdom.getGlory() - 9);
